...
@Database(entities = arrayOf(ExerciseItem::class, WorkoutItem::class, Badge::class, Login::class, User::class), version = 12, exportSchema = false)
    ...
    private class ExerciseDatabaseCallback(
        ...
        override fun onOpen(db: SupportSQLiteDatabase) {
            ...
            INSTANCE?.let { database ->
                scope.launch {
                    populateDatabase(database.exerciseDao(), database.workoutDao(), database.badgeDao(), database.loginDao(), database.userDao() )
        ...
        suspend fun populateDatabase(exerciseDao: ExerciseDao, workoutDao:WorkoutDao, badgeDao:BadgeDao, loginDao: LoginDao, userDao: UserDao) {
            ...
            var workout = WorkoutItem(
                R.drawable.ic_baseline_fitness_center_24, 0,
            ...
            // LOGINS and USERS
            var login = Login(loginName = "Admin", loginPassword = "password")
            var user = User(userName = "Admin")
            loginDao.insertLogin(login)
            userDao.insertUser(user)

            login = Login(loginName = "janedoe", loginPassword = "1234")
            user = User("Jane Doe")
            loginDao.insertLogin(login)
            userDao.insertUser(user)
...
//                        "4. Jump forward with your feet so that they land just outside of your hands\n" +
//                        "5. With your arms, reach over your head and jump up into the air\n" +
//                        "6. Land the jump and repeat for every rep\n"
//            ),
//
//            ExerciseItem(
//                R.drawable.ic_baseline_fitness_center_24,
...
//                            "6. Repeat for every rep\n"
//                )
//            ) // END of initial set of exercises
//
//
//
